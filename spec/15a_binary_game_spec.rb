...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        incorrect_value_one = 'a'
        incorrect_value_two = '50'
        valid_input = '5'
        allow(game_input).to receive(:gets).and_return(incorrect_value_one, incorrect_value_two, valid_input)
      end
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        ...
        max = game_input.instance_variable_get(:@maximum)
        user_input = '5'
        result = game_input.verify_input(min, max, user_input.to_i)
  ...
  describe '#display_turn_order' do
    ...
    it 'increases guess_count by one' do
      expect { game_turn.display_turn_order(search_display) }.to change { game_turn.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(search_display).to receive(:make_guess).once
      game_turn.display_turn_order(search_display)
    end
